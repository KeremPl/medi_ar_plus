    ...
    if (provider.hataMesaji != null && provider.egitimDetay == null) {
      return Center(
          child: Padding(
              ...
              child: Text(provider.hataMesaji!,
                  textAlign: TextAlign.center,
                  style: MetinStilleri.govdeMetni
                      .copyWith(color: Renkler.hataRengi))));
    }
    if (provider.egitimDetay == null || provider.egitimDetay!.adimlar.isEmpty) {
      return Center(
          child: Text('Eğitim adımları yükleniyor veya bulunamadı.',
              style: MetinStilleri.govdeMetniIkincil));
    }
    if (_pageController.hasClients &&
        _pageController.page?.round() != provider.mevcutAdimIndex) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (_pageController.hasClients) {
      ...
      Expanded(
          child: PageView.builder(
              ...
              itemBuilder: (context, index) {
                final EgitimAdimModel adim =
                    provider.egitimDetay!.adimlar[index];
  ...
  Widget _buildAdimSayfasi(EgitimAdimModel adim, BuildContext context) {
    ...
    if (adim.adimFotograf != null && adim.adimFotograf!.isNotEmpty) {
      ...
      if (gelenYol.startsWith('/images/egitim_adimlari/')) {
        ...
        tamFotografPath += '/images/egitim_adimlari/' + konuVeResim;
      } else {
                ...
                child: Container(
                    ...
                    child: ClipRRect(
                        ...
                        child: tamFotografPath.toLowerCase().endsWith('.svg')
                            ? SvgPicture.network(tamFotografPath,
                                fit: BoxFit.contain,
                                placeholderBuilder: (_) => const Center(
                                    child: CircularProgressIndicator(
                                        strokeWidth: 2.0)))
                            : Image.network(tamFotografPath,
          ...
          Expanded(
              ...
              child: SingleChildScrollView(
                  child: Padding(
                      ...
                      child: (adim.adimAciklama != null &&
                              adim.adimAciklama!.isNotEmpty)
                          ? Text(adim.adimAciklama!,
                              style: MetinStilleri.govdeMetni.copyWith(
                                  fontSize: 17,
                                  height: 1.65,
                          ...
                          : Center(
                              child: Text(
                                  "Bu adım için açıklama bulunmamaktadır.",
                                  style: MetinStilleri.govdeMetniIkincil)))))
        ]));
  }

  Widget _buildBottomButton(
      EgitimDetayProvider provider, BuildContext context) {
    bool sonAdim = provider.sonAdimdaMi;
    return Container(
      padding: const EdgeInsets.all(16.0),
      decoration: BoxDecoration(color: Renkler.kartArkaPlanRengi, boxShadow: [
        BoxShadow(
      ...
      child: ElevatedButton(
            ...
            Navigator.pushReplacement(
                ...
                MaterialPageRoute(
                    builder: (_) => EgitimTamamlamaEkrani(
                        egitimAdi: widget.egitimAdi, testId: widget.testId)));
          } else {
            provider.sonrakiAdimaGec();
            if (_pageController.hasClients) {
              _pageController.animateToPage(provider.mevcutAdimIndex,
        ...
        style: ElevatedButton.styleFrom(
            ...
            backgroundColor:
                sonAdim ? Renkler.yardimciRenk : Renkler.vurguRenk),
        child: Text(sonAdim ? 'Eğitimi Bitir' : 'Sonraki Adım',
            style: MetinStilleri.butonYazisi
                .copyWith(color: Renkler.butonYaziRengi)),
      ),
    );
  }
}
